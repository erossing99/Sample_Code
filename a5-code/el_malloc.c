...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  if (block != el_ctl.heap_start){ //checks if first block in heap
    el_blockfoot_t *lowerFoot = PTR_MINUS_BYTES(block,sizeof(el_blockfoot_t));
    el_blockhead_t *lower = PTR_MINUS_BYTES(lowerFoot,lowerFoot->size + sizeof(el_blockhead_t));
    return lower;
  }
  return NULL;
}
...
el_blockhead_t *el_find_first_avail(size_t size){
  int neededSize = size + EL_BLOCK_OVERHEAD;
  el_blockhead_t *currNode = el_ctl.avail->beg->next;  //moving my pointer to next in order to get first real node (not dummy node)
  while(currNode->state != 'E' && currNode->size < neededSize){
    currNode = currNode->next;
  }
  if (currNode->state == 'E'){ //check to make sure didn't reach end of list
    return NULL;
  }
  else{
    return currNode;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  el_blockfoot_t *foot = el_get_footer(block);
  foot->size = new_size;
  el_blockhead_t *newHead = el_block_above(block);
  newHead->size = oldsize - (EL_BLOCK_OVERHEAD + new_size);
  el_blockfoot_t *newFoot = el_get_footer(newHead);
...
void *el_malloc(size_t nbytes){
  el_blockhead_t *firstAvailable = el_find_first_avail(nbytes);
  if (firstAvailable == NULL){
    return NULL;
  }
  el_remove_block(el_ctl.avail,firstAvailable);
  el_blockhead_t *leftoverSpace = el_split_block(firstAvailable,nbytes);
  if (leftoverSpace == NULL){
    return NULL;
  }
  leftoverSpace->state = EL_AVAILABLE;
  firstAvailable->state = EL_USED;
  el_add_block_front(el_ctl.used,firstAvailable);
  el_add_block_front(el_ctl.avail,leftoverSpace);
  void *userPointer = PTR_PLUS_BYTES(firstAvailable,sizeof(el_blockhead_t));
  return userPointer;
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  el_blockfoot_t *newFoot = el_get_footer(higher);
  newFoot->size = lower->size;
  higher->size =-40;
...
void el_free(void *ptr){
  el_blockhead_t *precHead = PTR_MINUS_BYTES(ptr,sizeof(el_blockhead_t));
  if(precHead->state == EL_AVAILABLE){
    return;
  }
  el_remove_block(el_ctl.used,precHead);
  precHead->state = EL_AVAILABLE;
  el_add_block_front(el_ctl.avail,precHead);
  el_blockhead_t *nextBlock = el_block_above(precHead);
  el_blockhead_t *lowerBlock = el_block_below(precHead);
  if(nextBlock != NULL && nextBlock->state == EL_AVAILABLE){
    el_merge_block_with_above(precHead);
  }
  if(lowerBlock != NULL && lowerBlock->state == EL_AVAILABLE){
    el_merge_block_with_above(lowerBlock);
  }
  return;
}
